...
const DropDown = (props) => {
  ...
  {is_open && (
    ...
    >
      {values.map((text) => (
              <li>
                <a onClick={(e)=>{setState(e.target.innerText);Set_is_open(false);}}>{text}</a>
              </li>
            )
      )}
    </ul>
...
const handleFileChange = (event) => {
  ...
  useEffect(() => {
    ...
    if (user) { // user가 null이 아닐 때만 실행
      console.log(user);
      Set_mbti(user.user_mbti || "");
      Set_job(user.user_job || "");
      Set_study(user.user_study_field || "");
    }
    return () => {
            ...
            <div className="flex flex-col items-start">
              ...
              <div className="flex flex-row justify-center mb-6">
                ...
                <div className="mb-8">
                  ...
                  <input type="text" placeholder="이름" className="input w-full rounded-lg bg-[rgb(217,217,217)]" value={user && user.user_nickname ? user.user_nickname : ""}/>
                  <input type="text" placeholder="나이" className="input w-full rounded-lg bg-[rgb(217,217,217)]" value={user && user.user_age ? user.user_age : ""}/>
                  ...
                  <button className="btn btn-block btn-info text-white">입력완료</button>
                  </div>
    ...
    export default function Page() {
      ...
      return (
        <LoginChecker setUserData={setUserData}>
            <Edit_profile_Page user={userData} setUserData={setUserData} />
          </LoginChecker>
      );}