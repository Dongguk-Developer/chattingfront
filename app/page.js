...
const HelloUser = ({username})=>{
  ...
  <div className="font-black">
    <span className="text-xs"><p style={{display:"inline"}}>{username}</p><p className="text-[rgb(128,128,128)]" style={{display:"inline",margin:"0"}}>님</p></span>
    <p>안녕하세요!</p>
  </div>
...
const ChatRooms = ()=>{
  ...
  <div className="flex flex-col space-y-3 bg-[rgb(237,237,237)] p-2 rounded-lg shadow-md">
    ...
    {ChatRoom_list.map((chatroom,index)=>{
        return (<><div className="flex items-center p-2 rounded-lg bg-[rgb(217,217,217)] shadow" onClick={()=>document.getElementById(chatroom.room_code).showModal()}>
        ...
        <div className="flex-1">
          ...
          {chatroom.hashtags.map((tag, index) => (
            <p className="text-xs text-gray-500" style={{display:"inline"}}>{tag} </p>
          ))}
        </div>
      </div>
